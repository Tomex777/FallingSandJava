...
- Follow-up fix pass: initialize stage viewports before frame one; add outside-tap dismissal for the creator menu; unproject a fixed screen anchor through the menu's viewport; correct workflow taps and add visibility assertions for the menu, submenu, and native dialogs. Build and emulator retest pending.
- Run #55 (`58cca0d`) passed the startup and picker-size assertions and reached the save check with the Mouse Modes tap corrected to `(400, 90)`, but `files/save` was still absent after the workflow's Save attempt. Treat this as a remaining menu/action hit-test failure until the Run #55 save captures identify the exact row and dialog state.
- Run #55 captures confirm outside-tap dismissal works and the Tools button returns. However, reopening the menu shifts its rows down by 20 px: the first root menu starts at screen y≈40, while the second starts at y≈60 despite the same toolbar tap. Thus `(520, 70)` lands on Elements on the second open, and `(520, 130)` lands on Body Type; the screenshot sequence never proves a Save selection. Make the Tools menu use its stage viewport coordinates for a stable top-right anchor, then rerun the full flow with coordinates based on that fixed placement.
- Run #56 (`8d6d141`) built successfully and again reached the save assertion, but no save file was present. The menu now uses a fixed stage-viewport anchor; inspect its fresh captures before deciding whether the remaining issue is row coordinates or action dispatch.
- Run #56 captures show the direct world-coordinate anchor was wrong: the root menu rendered around y≈100–220, and the supposed Mouse Modes capture showed only the Tools button after the following tap dismissed the menu. Later, the Save tap opened Mouse Modes. The anchor must be computed by unprojecting the desired screen position through the menu stage's actual viewport, and the screenshots must be inspected before trusting file-size-only menu checks.
